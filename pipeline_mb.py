...
def main():
    ...
    if flux:
        if not os.path.exists(ctimg):
            fluximage(evtgticcd,merge+'/%s'%(obsid),binsize=binf,nproc=core,units='time')
            saida = os.path.join(fluxroot,'sb_')
            fluximage(evtgticcd,saida,binsize=binf,band='0.7:2:1.5',nproc=core)
        ...
        #         fluximage(evtgticcd,saida,bands=eband,binsize=binf,nproc=core,units='time')
    ...
    if blk:
        ...
        # for emin, emax in bandkE:
        #     bgimgi = os.path.join(fluxroot,"bg_{0}-{1}".format(emin,emax))
        #     img_in = os.path.join(fluxroot,"flux_{0}-{1}_thresh.img".format(emin,emax))
        #     if not os.path.exists(bgimgi):
        #         blank_image(blkevt,img_in,bgimgi)
    ...
    if src:
        ...
        # dmextract(infile=ctimg_mask+"[bin sky=@%s]"%(region),outfile=rprof_file1,exp=emap,bkg=bgimg+"[bin sky=@%s]"%(region),opt="generic",clobber=True)
        nbins = doSNR(rprof_file1,0.5,xcen,ycen,region.split('.reg')[0]+'_grp.reg')

        dmtcalc(rprof_file,rprof_file1.split('.fits')[0]+'_rmid.fits',expression='rmid=0.5*(r[0]+r[1])',clobber=True)
        ## Binning in groups of 20 counts
        # if nbins < 10:
        #     nbins = doGRP(rprof_file1,20,xcen,ycen,grpregion)
        rbkg, bgscale = rsource(rprof_file1)
    ...
    if rprof:
        ...
        # plotSB(infgtempl,inbgtempl,bgscale,plotname)

        # for emin, emax in bandkE:
        #    xpeak,ypeak= os.path.join(profroot,'total_fg_%04i_%04i_profile.dat'%(1000*emin,1000*emax))
        #     inbgtempl = os.path.join(profroot,'total_bg_%04i_%04i_profile.dat'%(1000*emin,1000*emax))
        #
        #     fimgi = os.path.join(fluxroot,"flux_{0}-{1}_thresh.img".format(emin,emax))
    ...
    """
    if arf:
        region_arf = os.path.join(root,"%s"%(obsid),"repro","%s_source.reg"%(obsid))
        arf_out = os.path.join(specroot,"%s"%(obsid))

        dmcoords(evtgti, asol="non", option="cel", ra=args.Xra, dec=args.Xdec, verbose=1)
        xobs, yobs = float(dmcoords.x), float(dmcoords.y)
        abertura(xobs,yobs,binf*rbkg,region_arf)
        if not os.path.exists(specroot):
            os.makedirs(specroot)
            arf_rmf(evtgti+"[sky=region(%s)]"%(region_arf),arf_out)
    ...
    if fit:
        ...
        plotSB3(rprof,rbkg,betapars,args.name)
        print("Tempo parcial: %s seconds" % (time.time() - t0))
