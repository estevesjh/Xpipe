...
def main():
    parser = argparse.ArgumentParser(
        description='X-ray Pipeline - Data Reduction',
    ...
    ## -------------------------------
    ## Flags
    down=1     ## Download the data
    reproc=1;  ## reprocess the data to lvl. 2
    flares=1;  ## clean fares
    flux=1;    ## Make flux images files
    blk=1;     ## Make blank files
    poisson=1; ## Add noise in the count images
    ps=1;      ## Find point sources
    cen=1;     ## Estimate the center and the errors
    src=1;     ## Make radial profiles; Define background region
    rprof=1;
    cshift=1;
    conce=1;
    save=1;
    ...
    ## -------------------------------
    if nobs > 1:
        mode = 'MOBS'   # Multiple observations mode
    else:
        mode = 'SOBS'   # Single observation mode
    ## -------------------------------
    if 2*the500kpc < 3*60:
    ...
    evt = os.path.join(merge,name+'evt.fits')
    evt2 = os.path.join(merge,name+'evt_gti.fits')
    ...
    print("--------------------------------------------")
    print("Running in",mode," mode, the aperture mode is",aperture)
    ...
    if blk:
        ...
        enu = ['(img%i)'%(j) for j in range(1,nobs+1)]
        ope = '('+'+'.join(enu)+')/%i'%(nobs)
        ...
        #     emaplist = [os.path.join(fluxroot,"flux_{0}_{1}-{2}_thresh.img".format(id,emin,emax)) for id in args.obsids]
